...
- 页面地址、菜单元数据和可选权限资源键统一声明在 `src/constants/routes.tsx`。资源键集中维护在 `src/constants/resources.ts`。
...
| 区域 | 规则 | 正确修改方式 |
| --- | --- | --- |
| `src/constants/routes.tsx` 的 `XCODEAGENT_BUSINESS_ROUTE_IMPORTS_START/END` 与 `XCODEAGENT_BUSINESS_ROUTES_START/END` 标记区 | 业务路由导入和声明只能追加在对应标记之间；保留标记及其外部的基础授权路由。 | 在标记区内新增页面导入和 `PageRouteDefinition` 配置。 |
...
3. 在 `src/constants/routes.tsx` 的业务标记区注册路由、菜单信息；只有授权清单已绑定资源时才填写 `resourceKey`。
