...
function AuthStateView({ state }: { state: AuthState }) {
  ...
  if (state === 'unauthenticated') return <Result status="403" title="登录状态已失效" subTitle="请重新登录后再访问系统权限管理。" />;
  if (state === 'forbidden') return <Result status="403" title="无权限访问" subTitle="当前成员没有系统权限管理资源。" />;
